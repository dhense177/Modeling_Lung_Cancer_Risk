...
Recently there has been increased focus on considering the interplay between many different health and environmental factors when determining risk for many illnesses. For example, a heavy smoker exposed to harmful air pollution is at significantly greater risk for lung cancer than a heavy smoker not exposed to harmful air pollution or a nonsmoker exposed to harmful air pollution. By looking at a wide variety of the known variables related to lung cancer, one might be able to better predict the number of to-be diagnosed lung cancer cases per year at a local level (county-wide), providing healthcare professionals and policymakers with actionable insights (e.g. estimated costs, cancer screening in high risk counties - early detection drastically increases chances of survival).
...
It is evident that counties differ drastically in their risk for lung cancer - counties in Kentucky show incidence rates of 150 people per 100,000, while counties in California hover around 30 per 100,000, a **5-fold difference.**
...
I requested research access to the NIH SEER Cancer Data, which comprises both cancer incidence and population data for several U.S. states from 1973-2014. I also found public county-wide data on adult smoking levels, radon levels, particulate matter levels (PM 2.5, PM 10), ozone levels, toxic releases and air quality index values. For this analysis I limited my time horizon to 2001-2011 due to the best data availability during this period. The full list of data sources used can be found in [data_dictionary.txt](https://github.com/dhense177/Modeling_Lung_Cancer_Risk/blob/master/data_dictionary.txt)


## ***Cleaning and Standardization***

A considerable amount of time was spent cleaning and grouping the SEER data so that it could be combined with the other data sources mentioned above.
...
When deciding which variables to include in my models, I compared the Bayesian Information Criteria (BIC) and Akaike Information Criteria (AIC) scores of various Lasso regressions that I ran, each including a different set of predictors:

![](Visuals/bic_table5.png)

### Table 1: Comparing Feature Sets Using BIC Scores

The BIC and AIC are very similar, except BIC penalizes extra parameters more harshly. I focused on the BIC.
...
Here it would be wise to consider what population we can infer from our sample counties. All U.S. Counties? Probably not. The data in this analysis is limited - we only have cancer data on counties in 7 states. Also, the health and environmental data I gathered tends to be more available in larger counties (>100,000 people). Therefore, it would make more sense to say that the relevant population is large U.S. counties or only large counties in the 7 states in the dataset (and maybe similar states).
...
One interesting finding is that there appears to be a negative correlation between Median Air Quality Index values and lung cancer incidence in these counties. This is a counterintuitive finding which certainly warrants further investigation. Confounding variables may be at play.
...
A lower RMSE value is desired. The fully-pooled model had an RMSE of **17.5**, and the unpooled an RMSE of **9.2**. Relative to the mean lung cancer incidence of ~70 people per 100,000 for all counties, the unpooled model is a reasonable approximation. Clearly the fully-pooled model is not a good option.
...
This type of parameter estimation is central to Bayesian Statistics. While
...
I tried 2 multilevel models, differing by the group distributions specified. The first uses state-level grouping, so that the population distribution for each county comprises all counties in the same state. The second model uses a population distribution comprising all 105 counties in the dataset.
...
Let's take a look at Warren County, KY to get a better sense for the difference between the 2 multilevel models I tried:
...
Kentucky counties have the highest lung cancer incidence out of all states in my dataset. The statewide average is ~100 per 100,000. Even though Warren County seems to show significantly lower mean incidence ~80-85, the dark green line is shifted upwards towards the Kentucky group mean. The dark blue line, however, fits the local data very well and seems less influenced by the overall mean for all counties (~70 per 100,000).
...
The estimation process is accomplished using the Markov Chain Monte Carlo (MCMC) algorithm. MCMC is gaining popularity in epidemiology where complex modeling is common, and can be a significant improvement over traditional maximum likelihood estimation techniques.
...
A full list of data sources can be found in [data_dictionary.txt](https://github.com/dhense177/Modeling_Lung_Cancer_Risk/blob/master/data_dictionary.txt). The sources below helped enormously with my analysis.
...
* [Data Sources for an Environmental Quality Index: Availability, Quality and Utility](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3222503/)

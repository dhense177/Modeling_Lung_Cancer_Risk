...
def group(df):
    df_grouped = df.groupby(['State-county recode', 'Race recode (White, Black, Other)','Sex','Year of diagnosis']).size().reset_index(name='Count')

    df_grouped['Combined'] = df_grouped['State-county recode'].astype(str)+df_grouped['Race recode (White, Black, Other)'].astype(str)+df_grouped.Sex.astype(str)
    ...
    df_grouped['Race recode (White, Black, Other)']=df_grouped.Combined.str[5:6]
    df_grouped['Sex']=df_grouped.Combined.str[6:]

    df_grouped.fillna(0,inplace=True)
    return df_grouped

def group_pop(df):
    pop_binned = pd.DataFrame(df.groupby(['State-county recode','Race','Sex','Year'])['Population'].sum().reset_index())
    pop_binned['Combined'] = pop_binned['State-county recode'].astype(str)+pop_binned.Race.astype(str)+pop_binned.Sex.astype(str)
    ...
    pop_final['Race']=pop_final.Combined.str[5:6]
    pop_final['Sex']=pop_final.Combined.str[6:]
    return pop_final
